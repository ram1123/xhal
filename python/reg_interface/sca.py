...
def main():
    ...
    if instructions == 'r':
        ...
        assert_hard_reset(ohMask)
    elif instructions == 'h':
        perform_hard_reset(ohMask)
...
def sca_reset(ohMask):
    subheading("Performing SCA reset...")
    ...
    subheading("SCA reset done.")

def perform_hard_reset(ohMask):
    if not checkStatusMask(ohMask):
        return
    subheading('Issuing FPGA Hard Reset')
    writeReg(getNode('GEM_AMC.SLOW_CONTROL.SCA.MANUAL_CONTROL.LINK_ENABLE_MASK'), ohMask)
    writeReg(getNode('GEM_AMC.SLOW_CONTROL.SCA.CTRL.OH_FPGA_HARD_RESET'), 0x1)
    
def assert_hard_reset(ohMask):
    if not checkStatusMask(ohMask):
        return
...
def checkStatusMask(ohMask):
    ohList = []
    for i in range(0,12):
        if check_bit(ohMask, i):
            ohList.append(i)
    return checkStatus(ohList)
